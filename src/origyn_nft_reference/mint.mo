                ...
                switch (library) {
                    case (#Array(item)) {
                        switch (item) {
                            case (#thawed(classes)) {
                                ...
                                for (this_item in classes.vals()) {
                                    ...
                                    let library_id = switch (Metadata.get_nft_text_property(this_item, Types.metadata.library_id)) {
                                        case (#err(err)) {
                                            return #err(Types.errors(#malformed_metadata, "handle_library - library needs library_id", ?caller));
                                        };
                                        case (#ok(val)) { val };
                                    };
                                    ...
                                    let library_size = switch (Metadata.get_nft_nat_property(this_item, Types.metadata.library_size)) {
                                        case (#err(err)) {
                                            return #err(Types.errors(#malformed_metadata, "handle_library - library needs size", ?caller));
                                        };
                                        case (#ok(val)) { val };
                                    };
                                    ...
                                    let library_type = switch (Metadata.get_nft_text_property(this_item, Types.metadata.library_location_type)) {
                                        case (#err(err)) {
                                            return #err(Types.errors(#malformed_metadata, "handle_library - library needs type", ?caller));
                                        };
                                        case (#ok(val)) { val };
                                    };
                            ...
                            case (_) {
                                return #err(Types.errors(#malformed_metadata, "stage_nft_origyn - library should be thawed", ?caller));
                            };
                        };
                    };
                    case (_) {
                        return #err(Types.errors(#malformed_metadata, "stage_nft_origyn - library should be an array", ?caller));
                    };
                };
        ...
        switch (Map.get(state.state.nft_metadata, Map.thash, id_val)) {
            case (null) {
                ...
                found_metadata := #Class(
                    switch (Properties.updateProperties(Conversions.valueToProperties(metadata), [{ name = Types.metadata.__system; mode = #Set(#Class([{ name = Types.metadata.__system_status; value = #Text(Types.nft_status_staged); immutable = false }])) }])) {
                        case (#err(errType)) {
                ...
                if (Metadata.is_minted(this_metadata) == false) {
                    ...
                    switch (Properties.getClassProperty(this_metadata, Types.metadata.__system)) {
                        ...
                        case (?found) {
                            ...
                            found_metadata := #Class(
                                switch (Properties.updateProperties(Conversions.valueToProperties(metadata), [{ name = Types.metadata.__system; mode = #Set(found.value) }])) {
                                    case (#err(errType)) {
                                        return #err(Types.errors(#update_class_error, "stage_nft_origyn - set staged status", ?caller));
                                    };
                                    case (#ok(result)) {
                        ...
                        new_metadata := switch (
                            if (this_item.immutable == true) {
                                Properties.updateProperties(
                                    Conversions.valueToProperties(new_metadata),
                                    [
                                        {
                                            name = this_item.name;
    ...
    public func stage_library_nft_origyn(
        ...
        if (bUpdate.size() > 0 or bDelete) {
            ...
            var found_metadata = #Class(
                switch (Properties.updateProperties(Conversions.valueToProperties(metadata), [{ name = Types.metadata.library; mode = #Set(#Array(#thawed(new_library.toArray()))) }])) {
                    case (#err(errType)) {
                        switch (errType) {
                            case (_) {
        ...
        switch (response) {
            case (#ok(result)) {
                ...
                var fresh_allocation = switch (Map.get<(Text, Text), Types.AllocationRecord>(refresh_state.state.allocations, (NFTUtils.library_hash, NFTUtils.library_equal), (chunk.token_id, chunk.library_id))) {
                    case (null) {
                        return #err(Types.errors(#not_enough_storage, "stage_library_nft_origyn_remote - allocation not found for " # chunk.token_id # " " # chunk.library_id, ?caller));
                    };
                    case (?val)(val);
                };
