...
module {
    ...
    public func stage_nft_origyn(
        ...
        let found_system = switch(Properties.getClassProperty(metadata, "__system")){
            case(null){};
            case(?found){
        ...
        switch(Map.get(state.state.nft_metadata, Map.thash, id_val)){
            ...
            case(?this_metadata){
                ...
                let system_node : CandyTypes.CandyValue = switch(Properties.getClassProperty(this_metadata, "__system")){
                ...
                let status : Text = Conversions.valueToText(
                    switch(Properties.getClassProperty(system_node, "status")){
                        case(null){return #err(Types.errors(#cannot_find_status_in_metadata, "stage_nft_origyn - find status", ?caller));};
                        case(?found){found.value};
                    });
    ...
    public func stage_library_nft_origyn(
        ...
        switch(chunk.filedata){
                        ...
                        switch(id.value){
                            ...
                            case(_){
                                return #err(Types.errors(#library_not_found, "stage_library_nft_origyn - provided filedata must be a claass with library_id as #Text attribute", ?caller));
                                    ...
                                    debug if(debug_channel.stage) D.print("new_library - " # library_id);
            ...
            case(_){
                return #err(Types.errors(#library_not_found, "stage_library_nft_origyn - cannot find library id in metadata - " # chunk.token_id #  " library_id: " # chunk.library_id, ?caller));
            };
        };
                    ...
                    switch(state.nft_library.get(chunk.token_id)){
                        case(null){
                            ...
                            if(chunk.content.size() > allocation.available_space){
                                                    debug if(debug_channel.stage) D.print("not enough storage in allocation null library " # debug_show(chunk.token_id, chunk.library_id, chunk.content.size(),allocation.available_space));
                    ...
                    for(this_index in Iter.range(SB.size<Nat>(allocation.chunks), chunk.chunk)){
                       //D.print(debug_show(this_index));
                        if(this_index == chunk.chunk){
                            if(chunk.content.size() > allocation.available_space){
                                                    debug if(debug_channel.stage) D.print("not enough storage in allocation not branch b" # debug_show(chunk.token_id, chunk.library_id, chunk.content.size(),allocation.available_space));
                                ...
                                return #err(Types.errors(#not_enough_storage, "stage_library_nft_origyn - chunk bigger than available past workspace" # chunk.token_id # " " # chunk.library_id, ?caller));
                                                ...
                                                debug if(debug_channel.stage) D.print("branch c" # debug_show(allocation, chunk.content.size()));
            ...
            case(#ok(result)){
                ...
                if(chunk.chunk + 1 <= SB.size<Nat>(fresh_allocation.chunks)){
                    ...
                    if(chunk.content.size() > current_size){
                        ...
                        SB.put<Nat>(fresh_allocation.chunks, chunk.chunk, chunk.content.size());
                        fresh_allocation.available_space += (chunk.content.size() - current_size);
                    } else if(chunk.content.size() != current_size){
                        //give space back
                        SB.put<Nat>(fresh_allocation.chunks, chunk.chunk, chunk.content.size());
                        fresh_allocation.available_space -= (current_size - chunk.content.size());
                    } else {};
                } else {
                    for(this_index in Iter.range(SB.size<Nat>(fresh_allocation.chunks), chunk.chunk)){
                        if(this_index == chunk.chunk){
                            SB.add<Nat>(fresh_allocation.chunks, chunk.content.size());
                            fresh_allocation.available_space += chunk.content.size();
                        } else {
