   ...
   public func validateDeposit(host: Principal, deposit : Types.DepositDetail, caller: Principal) : async Result.Result<Bool, Types.OrigynError> {
        ...
        try{
           //D.print("sending transfer blocks # " # debug_show(escrow.deposit.amount - ledger.fee));
            ...
            let result_block = switch(result){
                ...
                case(#err(err)){
                    return #err(Types.errors(#validate_deposit_failed, "ledger_interface - transfer deposit failed " # debug_show(escrow.deposit) # " " # debug_show(err), ?caller));
                };
            };
            ...
            return #err(Types.errors(#validate_deposit_failed, "ledger_interface - validate deposit - ledger throw " # Error.message(e) # debug_show(escrow.deposit), ?caller));
        };
    };
    ...
    public func send_payment_minus_fee(account: Types.Account, token: Types.ICTokenSpec, amount : Nat, sub_account: ?Blob, caller: Principal) : async Result.Result<{trx_id: Types.TransactionID; fee: Nat}, Types.OrigynError> {
        ...
        try{
            ...
            switch(result){
                case(#Ok(val)){
                    #ok({trx_id = #nat(Nat64.toNat(val)); fee = token.fee});
                };
                case(#Err(err)){
                    #err(Types.errors(#nyi, "ledger_interface - send payment - payment failed " # debug_show(err), ?caller));}
                };
        } catch (e){
            return #err(Types.errors(#nyi, "ledger_interface - send payment - payment failed " # Error.message(e), ?caller));}
    };
