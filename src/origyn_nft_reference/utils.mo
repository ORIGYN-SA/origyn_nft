...
module {
  ...
  public func get_nat_as_token_id(tokenNat : Nat) : Result.Result<Text, Types.OrigynError> {
    ...
    } else if (staged.size() % 4 == 1) { CandyTypes.toBuffer<Nat8>([0, 0, 0]) } else if (staged.size() % 4 == 2) {
  ...
  public func is_owner_manager_network(state : Types.State, caller : Principal) : Bool {
    ...
    debug if (debug_channel.announce) debug {
      D.print("checking if " # Principal.toText(caller) # " is network:" # debug_show (state.state.collection_data.network) # " owner: " # debug_show (state.state.collection_data.owner) # " manager: " # debug_show (state.state.collection_data.managers));
    };
    if (caller == state.state.collection_data.owner) { return true };
    if (Array.filter<Principal>(state.state.collection_data.managers, func(item : Principal) { item == caller }).size() > 0) {
      return true;
    };
    if (Option.make(caller) == state.state.collection_data.network) {
      return true;
    };
