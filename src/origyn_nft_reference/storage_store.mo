  ...
  */
  public func stage_library_nft_origyn(
    ...
    if (state.state.collection_data.owner != caller) {
      return #err(Types.errors(null, #unauthorized_access, "stage_library_nft_origyn - storage - not the gateway", ?caller));
    };
    ...
    let allocation = switch (Map.get<(Text, Text), Types.AllocationRecord>(state.state.allocations, (NFTUtils.library_hash, NFTUtils.library_equal), (chunk.token_id, chunk.library_id))) {
      case (null) {
        ...
        Map.set<(Text, Text), Types.AllocationRecord>(
          state.state.allocations,
          (NFTUtils.library_hash, NFTUtils.library_equal),
          (chunk.token_id, chunk.library_id),
          allocation,
        );
