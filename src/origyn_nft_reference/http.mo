...
module {
  ...
  public func handle_stream_content(
    ...
    let canister = actor (canister_id) : actor {
      nftStreamingCallback : shared () -> async ();
    };
  ...
  private func _redirect_to_storage_canister(
    ...
    location : Text,
    use_token_id : Text,
    allocation_canister : Principal,
  ) : HTTPResponse {
    debug if (debug_channel.library) D.print("item is not on this server redir to " # Principal.toText(allocation_canister));
    let #ok(location) = Metadata.get_nft_text_property(library_meta, "location") else return _not_found("location not found" # token_id # " " # library_id);
  ...
  public func renderLibrary(
    ...
    let #ok(location_type) = Metadata.get_nft_text_property(library_meta, "location_type") else return _not_found("location type not found" # token_id # " " # library_id);
    ...
    let ?allocation = Map.get<(Text, Text), Types.AllocationRecord>(
      ...
      (use_token_id, library_id),
    ) else return _not_found("allocation for token, library not found - " # use_token_id # " " # library_id);
  ...
  public func get_deep_properties(metadata : CandyTypes.CandyShared, qs : List.List<sQuery>) : {
    #ok : CandyTypes.CandyShared;
    #err;
    #back;
  } {
    if (List.isNil(qs)) {
          ...
          if (path_size >= 3) {
            ...
            if (path_array[2] == "translate") {
              ...
              let translation = #Class([
                ...
                {
                  name = "ext";
                  value = #Text(Types._getEXTTokenIdentifier(token_id, state.canister()));
                  immutable = true;
                },
                {
                  name = "dip721";
                  value = #Text(Nat.toText(NFTUtils.get_token_id_as_nat(token_id)));
                  immutable = true;
                },
              ]);
              ...
              if (path_array[2] == "-") {
                ...
                if (path_size >= 5 and path_array[path_array.size() -1] == "info") {
                  let library_meta = switch (Metadata.get_library_meta(metadata, library_id)) {
                    case (#err(err)) {
                      return _not_found("library by " # library_id # " not found");
                    };
                    case (#ok(val)) { val };
                  };
            ...
            let translation = Array.map<Text, CandyTypes.CandyShared>(
              ...
              func(x) {
                ...
                return #Class([
                  ...
                  {
                    name = "ext";
                    value = #Text(Types._getEXTTokenIdentifier(x, state.canister()));
                    immutable = true;
                  },
                  {
                    name = "dip721";
                    value = #Text(Nat.toText(NFTUtils.get_token_id_as_nat(x)));
                    immutable = true;
                  },
                ]);
              },
