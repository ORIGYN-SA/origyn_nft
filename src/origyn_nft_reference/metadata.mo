...
module {
    ...
    for (thisItem in Map.vals<MigrationTypes.Current.AskFeatureKey, MigrationTypes.Current.AskFeature>(items)) {
      switch (thisItem) {
        ...
        case (#allow_list(e)) {
          candy_buffer.add({
            name = "allow_list";
            value = #Array(Array.map<Principal, CandyTypes.CandyShared>(e, func(x : Principal) { #Principal(x) }));
            immutable = true;
          });
        };
        case (#notify(e)) {
        ...
        case (#ending(e)) {
          candy_buffer.add(
            switch (e) {
              case (#date(val)) {
                { name = "ending_date"; value = #Int(val); immutable = true };
              };
              case (#timeout(val)) {
                { name = "ending_timeout"; value = #Nat(val); immutable = true };
              };
            }
          );
        };
        ...
        case (#fee_accounts(e)) {
          candy_buffer.add({
            name = "fee_accounts";
            value = #Array(
              Array.map<Text, CandyTypes.CandyShared>(
                e,
                func(x : Text) {
                  #Text(x);
                },
      ...
      if (this_entry.name == Types.metadata.__system) {
        ...
        if (app_nodes.size() > 0) {
          final_object.add({
            ...
            value = #Array(Buffer.toArray(app_nodes));
            immutable = false;
          });
        };
      } else {
                    ...
                    switch (read_type.value) {
                      case (#Text(read_type_detail)) {
                        if (read_type_detail == "allow") {
                          ...
                          //nyi: implement block list; roles based security
  ...
  */
  ...
  ) : Result.Result<CandyTypes.CandyShared, Types.OrigynError> {
  ...
  */
  public func announceTransaction<system>(state : Types.State, rec : MigrationTypes.Current.TransactionRecord) : () {
    ...
    let (eventType, payload) = switch (rec.txn_type) {
      case (#auction_bid(data)) {
        ("auction_bid", #Class([{ name = "token_id"; value = #Text(rec.token_id); immutable = true }, { name = "canister"; value = #Principal(state.canister()); immutable = true }, { name = "sale_id"; value = #Text(data.sale_id); immutable = true }]));
      };
  ...
  */
  public func chunk_nft_origyn(state : Types.State, request : Types.ChunkRequest, caller : ?Principal) : Types.ChunkResult {
    ...
    if (allocation.canister != state.canister()) {
      ...
      return #ok(#remote({ canister = allocation.canister; args = { chunk = request.chunk; library_id = request.library_id; token_id = use_token_id } }));
  ...
  public func collection_update_nft_origyn(state : Types.State, request : Types.ManageCollectionCommand, caller : Principal) : Types.OrigynBoolResult {
    ...
    switch (request) {
      ...
      case (#UpdateMetadata(key, val, immutable)) {
        ...
        let insert_result = if (immutable == true) {
          Properties.updatePropertiesShared(Conversions.candySharedToProperties(current_metadata), [{ name = key; mode = #Lock(clean_val) }]);
        } else {
          Properties.updatePropertiesShared(Conversions.candySharedToProperties(current_metadata), [{ name = key; mode = #Set(clean_val) }]);
        };
            ...
            value = switch (thisItem.txn_type) {
              ...
              case (#royalty_paid(val)) {
                #Class([
                  ...
                  {
                    name = "receiver";
                    value = account_to_candy(val.receiver);
                    immutable = true;
                  },
              ...
              case (#sale_opened(val)) {
                #Class([
                  ...
                  {
                    name = "pricing";
                    value = pricing_shared_to_candy(val.pricing);
                    immutable = true;
                  },
              ...
              case (#canister_managers_updated(val)) {
                #Class([
                  ...
                  {
                    name = "managers";
                    value = #Array(Array.map<Principal, CandyTypes.CandyShared>(val.managers, func(x : Principal) { #Principal(x) }));
                    immutable = true;
                  },
                  {
                    name = "extensible";
