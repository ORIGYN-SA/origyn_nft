...
module {
        ...
        #Class([
            ...
            },
            {
                ...
                value = switch (val.allow_list) {
                    case (null) { #Empty };
                    case (?val) {
                        #Array(#frozen(Array.map<Principal, CandyType.CandyValue>(val, func(x : Principal) { #Principal(x) })));
                    };
                };
                immutable = true;
            },
    ...
    public func set_nft_owner(state : Types.State, token_id : Text, new_owner : Types.Account, caller : Principal) : Result.Result<CandyTypes.CandyValue, Types.OrigynError> {
        ...
        var temp_metadata : CandyTypes.CandyValue = switch (
            Properties.updateProperties(
                Conversions.valueToProperties(fresh_metadata),
                [
                    {
            ...
            if (this_entry.name == Types.metadata.__system) {
                ...
                if (app_nodes.size() > 0) {
                    final_object.add({
                        ...
                        value = #Array(#thawed(app_nodes.toArray()));
                        immutable = false;
                    });
                };
            } else {
    ...
    ) : Result.Result<CandyTypes.CandyValue, Types.OrigynError> {
    ...
    public func add_transaction_record(state : Types.State, rec : Types.TransactionRecord, caller : Principal) : Result.Result<Types.TransactionRecord, Types.OrigynError> {
        ...
        // set a callback to pass the newTrx
        ...
        let result = switch (prevSubscriptionInfo) {
            ...
            case (?subscriberInfo) {
                ...
                if (eventSubscribed != null) {
                    ...
                    return announcements // or newTrx;
                } else {
                    return null;
                };
            };
        ...
        switch (request) {
            ...
            case (#UpdateMetadata(key, val, immutable)) {
                ...
                let insert_result = if (immutable == true) {
                    Properties.updateProperties(
                        Conversions.valueToProperties(current_metadata),
                        [
                            {
                                name = key;
                                mode = #Lock(clean_val);
                    ...
                    Properties.updateProperties(
                        Conversions.valueToProperties(#Class([])),
                        [
                            {
                                name = key;
                                mode = #Set(clean_val);
            ...
            case (#UpdateManagers(data)) {
                ...
                NFTUtils.add_log(
                    state,
                    {
                        ...
                        data = #Array(#frozen(Array.map<Principal, CandyTypes.CandyValue>(data, func(item) { #Principal(item) })));
                        caller = ?caller;
                    },
                );
                        ...
                        value = switch (thisItem.txn_type) {
                            ...
                            case (#canister_managers_updated(val)) {
                                #Class([
                                    ...
                                    {
                                        name = "managers";
                                        value = #Array(#frozen(Array.map<Principal, CandyType.CandyValue>(val.managers, func(x : Principal) { #Principal(x) })));
                                        immutable = true;
                                    },
                                    {
                                        name = "extensible";
