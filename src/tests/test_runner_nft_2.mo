...
shared (deployer) actor class test_runner(dfx_ledger : Principal, dfx_ledger2 : Principal) = this {
  ...
  public shared func test(canister_factory : Principal, storage_factory : Principal) : async {
    ...
    let suite = S.suite(
      ...
      [
        ...
        S.test("testBuyItNow_v2", switch (await testBuyItNow_v2()) { case (#success) { true }; case (_) { false } }, M.equals<Bool>(T.bool(true))),
      ],
    );
    S.run(suite);
  ...
  public shared func testStage() : async { #success; #fail : Text } {
    ...
    //MINT0010
    let fileStageResult = await canister.chunk_nft_origyn({
      token_id = "1";
      library_id = "page";
      chunk = ?0;
    });
    ...
    let fileStageResult2 = await canister.chunk_nft_origyn({
      token_id = "1";
      library_id = "page";
      chunk = ?1;
    });
    ...
    //MINT0004
    let fail_stage_because_id = await canister.stage_nft_origyn({
      metadata = #Class([
        ...
        { name = "hidden"; value = #Text("page"); immutable = true },
        {
          ...
          value = #Array([
            #Class([
              ...
              {
                name = "location";
                value = #Text("https://" # Principal.toText(Principal.fromActor(canister)) # ".raw.icp0.io/_/1/_/page");
                immutable = true;
              },
              {
                name = "content_type";
        ...
        {
          name = "owner";
          value = #Principal(Principal.fromActor(canister));
          immutable = false;
        },
      ]);
    });
    ...
    //MINT0006
    let fail_stage_because_system = await canister.stage_nft_origyn({
      metadata = #Class([
    ...
    let test_metadata_replace_command = await canister.stage_nft_origyn({
      metadata = #Class([
    ...
    let suite = S.suite(
      ...
      [
        ...
        S.test(
          "file stage query works",
          switch (fileStageResult) {
            case (#ok(res)) {
              switch (res) {
        ...
        S.test(
          "cant publish metadata for someone else",
          switch (a_wallet_try_publish) {
            case (#ok(res)) {
          ...
          M.equals<Text>(T.text("unauthorized access")),
        ), //MINT0014
        S.test(
          "cant publish file for someone else",
          switch (a_wallet_try_file_publish) {
            case (#ok(res)) {
              "shoundnt be able to publish";
            };
            case (#err(err)) {
        ...
        S.test(
          "can see metadata after I stage",
          switch (test_metadata) {
            case (#ok(res)) {
              switch (res.metadata) {
                case (#Class(data)) {
                  if (data.size() == 13) {
                    //check if a top level element was added to the structure
          ...
          M.equals<Text>(T.text("Ok")),
        ), //MINT0017
        S.test(
          "can't see metadata after stage from wallet",
          switch (a_wallet_try_get_nft) {
            case (#ok(res)) {
          ...
          M.equals<Text>(T.text("Cannot find token.")),
        ), //MINT0018
        S.test(
          "can't see bearer after stage from wallet",
          switch (a_wallet_try_get_bearer) {
            case (#ok(res)) {
              "shoundnt be able to get";
            };
            case (#err(err)) {
        ...
        S.test(
          ...
          M.equals<Text>(T.text("page2")),
        ), //MINT0005
        S.test(
          "cant upload library_id that doesnt exist metadata",
          switch (fileStage3) {
            case (#ok(res)) {
  ...
  public shared func testMint() : async { #success; #fail : Text } {
    ...
    let test_metadata_replace_command_with_system = await canister.stage_nft_origyn({
      metadata = #Class([
        { name = "id"; value = #Text("1"); immutable = true },
    ...
    let test_metadata_replace_command_with_library = await canister.stage_nft_origyn({
      metadata = #Class([
        { name = "id"; value = #Text("1"); immutable = true },
    ...
    let test_metadata_replace_command_with_owner = await canister.stage_nft_origyn({
      metadata = #Class([
        { name = "id"; value = #Text("1"); immutable = true },
    ...
    let test_metadata_replace_command_with_apps = await canister.stage_nft_origyn({
      metadata = #Class([
        { name = "id"; value = #Text("1"); immutable = true },
    ...
    let test_metadata_replace_command_with_immutable = await canister.stage_nft_origyn({
      metadata = #Class([
        { name = "id"; value = #Text("1"); immutable = true },
    ...
    let suite = S.suite(
      ...
      [
        ...
        S.test(
          "fail if non owner mints",
          switch (a_wallet_try_mint) {
        ...
        S.test(
          "creator can assign owner on mint",
          switch (bearer_attempt) {
            case (#ok(res)) {
              switch (res) {
          ...
          M.equals<Text>(T.text(Principal.toText(Principal.fromActor(a_wallet)))),
        ), //MINT0024
        S.test(
          "fail if a_wallet_try_publish_change",
          switch (a_wallet_try_publish_change) {
            case (#ok(res)) { "unexpected success" };
            case (#err(err)) {
              if (err.number == 2000) {
                "correct number";
        ...
        S.test(
          "user can see nft after update",
          switch (view_after_update_attempt) {
            case (#ok(res)) {
        ...
        S.test(
          "fail if test_metadata_replace_command_with_system",
          switch (test_metadata_replace_command_with_system) {
            case (#ok(res)) { "unexpected success" };
            case (#err(err)) {
              if (err.number == 2) {
                "correct number";
        ...
        S.test(
          "fail if test_metadata_replace_command_with_library",
          switch (test_metadata_replace_command_with_library) {
            case (#ok(res)) { "unexpected success" };
            case (#err(err)) {
              if (err.number == 1002) {
                "correct number";
        ...
        S.test(
          "fail if test_metadata_replace_command_with_owner",
          switch (test_metadata_replace_command_with_owner) {
            case (#ok(res)) { "unexpected success" };
            case (#err(err)) {
              if (err.number == 1002) {
                "correct number";
        ...
        S.test(
          "fail if test_metadata_replace_command_with_apps",
          switch (test_metadata_replace_command_with_apps) {
            case (#ok(res)) { "unexpected success" };
            case (#err(err)) {
              if (err.number == 1002) {
                "correct number";
        ...
        S.test(
          "fail if test_metadata_replace_command_with_immutable",
          switch (test_metadata_replace_command_with_immutable) {
            case (#ok(res)) { "unexpected success" };
            case (#err(err)) {
              if (err.number == 1000) {
                "correct number";
  ...
  public shared func testBuyItNow() : async { #success; #fail : Text } {
    ...
    let c_wallet_try_bid_valid = await c_wallet.try_bid(Principal.fromActor(canister), Principal.fromActor(this), Principal.fromActor(dfx), 10 * 10 ** 8, "1", current_sales_id, null, null);
    ...
    let a_wallet_try_bid_below_start = await a_wallet.try_bid(Principal.fromActor(canister), Principal.fromActor(this), Principal.fromActor(dfx), 1 * 10 ** 7, "1", current_sales_id, null, null);
    ...
    let b_wallet_try_bid_wrong_asset = await b_wallet.try_bid(Principal.fromActor(canister), Principal.fromActor(this), Principal.fromActor(dfx2), 10 * 10 ** 8, "1", current_sales_id, null, null);
    ...
    let a_wallet_try_bid_valid = await a_wallet.try_bid(Principal.fromActor(canister), Principal.fromActor(this), Principal.fromActor(dfx), (10 * 10 ** 8) + 1, "1", current_sales_id, null, null);
    ...
    let suite = S.suite(
      ...
      [
        ...
        S.test(
          "fail if bid not on allow list",
          switch (c_wallet_try_bid_valid) {
            case (#ok(res)) { "unexpected success" };
            case (#err(err)) {
              if (err.number == 2000) {
                "correct number";
        ...
        S.test(
          "nft balance after sale to bad user is 0",
          switch (c_balance_after_bad_bid) {
            case (#ok(res)) {
        ...
        S.test(
          "transaction history has the bid 1",
          switch (a_history_1) {
            case (#ok(res)) {
        ...
        S.test(
          "transaction history have the transfer",
          switch (a_history_3) {
            case (#ok(res)) {
  ...
  public shared func testBuyItNow_v2() : async { #success; #fail : Text } {
    D.print("running testBuyItNow");
    ...
    let suite = S.suite(
      ...
      [
        ...
        S.test(
          "transaction history has the bid 2",
          switch (a_history_1) {
            case (#ok(res)) {
  ...
  public shared func testKYC() : async { #success; #fail : Text } {
    ...
    //start an auction by owner
    let start_auction_attempt_owner_fail = await canister.market_transfer_nft_origyn({
      token_id = "2";
      sales_config = {
        escrow_receipt = null;
        broker_id = null;
    ...
    let a_wallet_try_bid_valid = await a_wallet.try_bid(Principal.fromActor(canister), Principal.fromActor(this), Principal.fromActor(dfx), (10 * 10 ** 8), "1", current_sales_id, null, null);

    D.print("a try bid " # debug_show (a_wallet_try_bid_valid));

    let a_balance_after_bad_bid4 = await canister.balance_of_nft_origyn(#principal(Principal.fromActor(a_wallet)));

    let dfx_a_balance_after_bad_bid = await dfx.icrc1_balance_of({
    ...
    let b_wallet_try_bid_valid = await b_wallet.try_bid(Principal.fromActor(canister), Principal.fromActor(this), Principal.fromActor(dfx), (10 * 10 ** 8) + 1, "1", current_sales_id, null, null);
    ...
    let suite = S.suite(
      ...
      [
        ...
        S.test(
          "fail if kyc doesn't fail on start_auction",
          switch (start_auction_attempt_owner_fail) {
            case (#ok(res)) { "unexpected success" };
            case (#err(err)) {
              if (err.number == 4011) {
                //wrong asset
          ...
          switch (b_wallet_try_bid_valid) {
            case (#ok(res)) {
              ...
              switch (res.txn_type) {
                case (#sale_ended(details)) {
                  if (
                    Types.account_eq(details.buyer, #principal(Principal.fromActor(b_wallet))) and
                    details.amount == ((10 * 10 ** 8) + 1) and
                    (switch (details.sale_id) { case (null) { "x" }; case (?val) { val } }) == current_sales_id and
                    Types.account_eq(details.seller, #principal(Principal.fromActor(this))) and
                    Types.token_eq(details.token, #ic({ canister = (Principal.fromActor(dfx)); standard = #Ledger; decimals = 8; symbol = "LDG"; fee = ?200000; id = null }))
        ...
        S.test(
          "transaction history has the bid 3",
          switch (a_history_3) {
            case (#ok(res)) {
