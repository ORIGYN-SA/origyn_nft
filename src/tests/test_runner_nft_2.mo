    ...
    public shared func testStage() : async { #success; #fail : Text } {
        ...
        let test_metadata_replace_command = await canister.stage_nft_origyn({
            metadata = #Class([
                ...
                {
                    name = "library";
                    value = #Array(#thawed([#Class([{ name = "library_id"; value = #Text("page2"); immutable = true }, { name = "title"; value = #Text("page"); immutable = true }, { name = "location_type"; value = #Text("canister"); immutable = true }, { name = "location"; value = #Text("https://" # Principal.toText(Principal.fromActor(canister)) # ".raw.ic0.app/_/1/_/page"); immutable = true }, { name = "content_type"; value = #Text("text/html; charset=UTF-8"); immutable = true }, { name = "content_hash"; value = #Bytes(#frozen([0, 0, 0, 0])); immutable = true }, { name = "size"; value = #Nat(4); immutable = true }, { name = "sort"; value = #Nat(0); immutable = true }, { name = "read"; value = #Text("public"); immutable = false }, ])]));
                    immutable = true;
                },
                {
                    name = "owner";
    ...
    public shared func testMint() : async { #success; #fail : Text } {
        ...
        let test_metadata_replace_command_with_apps = await canister.stage_nft_origyn({
            metadata = #Class([
                ...
                {
                    name = "__apps";
                    value = #Array(#thawed([#Class([{ name = "app_id"; value = #Text("page2"); immutable = true }, { name = "title"; value = #Text("page"); immutable = true }, { name = "location_type"; value = #Text("canister"); immutable = true }, { name = "location"; value = #Text("https://" # Principal.toText(Principal.fromActor(canister)) # ".raw.ic0.app/_/1/_/page"); immutable = true }, { name = "content_type"; value = #Text("text/html; charset=UTF-8"); immutable = true }, { name = "content_hash"; value = #Bytes(#frozen([0, 0, 0, 0])); immutable = true }, { name = "size"; value = #Nat(4); immutable = true }, { name = "sort"; value = #Nat(0); immutable = true }, { name = "read"; value = #Text("public"); immutable = false }, ])]));
                    immutable = true;
                },
                    ...
                    switch (bearer_attempt) {
                        case (#ok(res)) {
                            switch (res) {
                                ...
                                case (_) {
                                    "unexpected account type" # debug_show (res);
                                };
                            };
                        };
                        case (#err(err)) {
                    ...
                    switch (a_history_1) {
                        case (#ok(res)) {
                            ...
                            switch (res[res.size() -1].txn_type) {
                                case (#sale_ended(details)) {
                                    if (
                                        Types.account_eq(details.buyer, #principal(Principal.fromActor(a_wallet))) and details.amount == ((10 * 10 ** 8) + 1) and details.sale_id == ?current_sales_id and Types.token_eq(details.token, #ic({ canister = (Principal.fromActor(dfx)); standard = #Ledger; decimals = 8; symbol = "LDG"; fee = 200000 }))
                                    ) {
                                        "correct response";
                                    } else {
                                        "details didnt match" # debug_show (details);
                    ...
                    switch (b_wallet_try_bid_valid) {
                        case (#ok(res)) {
                            ...
                            switch (res.txn_type) {
                                case (#sale_ended(details)) {
                                    if (
                                        Types.account_eq(details.buyer, #principal(Principal.fromActor(b_wallet))) and details.amount == ((10 * 10 ** 8) + 1) and (switch (details.sale_id) { case (null) { "x" }; case (?val) { val } }) == current_sales_id and Types.account_eq(details.seller, #principal(Principal.fromActor(this))) and Types.token_eq(details.token, #ic({ canister = (Principal.fromActor(dfx)); standard = #Ledger; decimals = 8; symbol = "LDG"; fee = 200000 }))
                                    ) {
                                        "correct response";
                                    } else {
                                        "details didnt match" # debug_show (details);
                    ...
                    switch (a_history_3) {
                        case (#ok(res)) {
                            ...
                            switch (res[res.size() -1].txn_type) {
                                case (#sale_ended(details)) {
                                    if (
                                        Types.account_eq(details.buyer, #principal(Principal.fromActor(b_wallet))) and details.amount == ((10 * 10 ** 8) + 1) and details.sale_id == ?current_sales_id and Types.token_eq(details.token, #ic({ canister = (Principal.fromActor(dfx)); standard = #Ledger; decimals = 8; symbol = "LDG"; fee = 200000 }))
                                    ) {
                                        "correct response";
                                    } else {
                                        "details didnt match" # debug_show (details);
