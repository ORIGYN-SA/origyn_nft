...
shared (deployer) actor class test_runner(dfx_ledger : Principal, dfx_ledger2 : Principal) = this {
    ...
    public shared func testRoyalties() : async { #success; #fail : Text } {
        ...
        let a_balance = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(a_wallet), null));
        });
        let b_balance = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(b_wallet), null));
        });
        let n_balance = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(n_wallet), null));
        });
        let o_balance = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(o_wallet), null));
        });
        let canister_balance = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(canister), null));
        });
        let net_balance = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(net_wallet), null));
        });
        ...
        //BAL0005
        let a_balance2 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(a_wallet), null));
        });
        let b_balance2 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(b_wallet), null));
        });
        let n_balance2 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(n_wallet), null));
        });
        let o_balance2 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(o_wallet), null));
        });
        let canister_balance2 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(canister), null));
        });
        let net_balance2 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(net_wallet), null));
        });
        ...
        let a_balance3 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(a_wallet), null));
        });
        let b_balance3 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(b_wallet), null));
        });
        let n_balance3 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(n_wallet), null));
        });
        let o_balance3 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(o_wallet), null));
        });
        let canister_balance3 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(canister), null));
        });
        let net_balance3 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(net_wallet), null));
        });
        ...
        let a_balance5 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(a_wallet), null));
        });
        let b_balance5 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(b_wallet), null));
        });
        let n_balance5 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(n_wallet), null));
        });
        let o_balance5 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(o_wallet), null));
        });
        let canister_balance5 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(canister), null));
        });
        let net_balance5 = await dfx.account_balance_dfx({
            account = AccountIdentifier.toText(AccountIdentifier.fromPrincipal(Principal.fromActor(net_wallet), null));
        });
        ...
        let suite = S.suite(
            ...
            [
                ...
                S.test("fail if originator does not get first royalty", o_balance3.e8s, M.equals<Nat64>(T.nat64(3126633))), //S.test("fail if broker still has balance after withdraw", b_balance4.e8s, M.equals<Nat64>(T.nat64(6))),
    ...
    public shared func testAuction() : async { #success; #fail : Text } {
        ...
        let owner_withdraw_proper = await dfx.account_balance_dfx({
                        ...
                        case (#ok(res)) {
                            ...
                            switch (res[res.size() -2].txn_type) {
                                case (#sale_withdraw(details)) {
                                    ...
                                    if (
                                        Types.account_eq(details.buyer, #principal(Principal.fromActor(a_wallet))) and Types.account_eq(details.seller, #principal(Principal.fromActor(this))) and details.amount == (Nat.sub(((101 * 10 ** 8) -200000), dip20_fee)) and details.token_id == "1" and Types.token_eq(details.token, #ic({ canister = (Principal.fromActor(dfx)); standard = #Ledger; decimals = 8; symbol = "LDG"; fee = 200000 })),
                                    ) {
                                        "correct response";
                                    } else {
                                        "details didnt match" # debug_show (details);
