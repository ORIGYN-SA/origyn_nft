...
TEST_RUNNER_UTIL_CANISTER_ID=$(dfx canister id test_runner_utils)
TEST_RUNNER_UTIL_ACCOUNT_ID=$(python3 principal_to_accountid.py $TEST_RUNNER_UTIL_CANISTER_ID)
...
dfx canister install test_runner --mode=reinstall --wasm ./.dfx/local/canisters/test_runner/test_runner.wasm.gz --argument "(record { canister_factory = principal \"$TEST_CANISTER_FACTORY_ID\"; storage_factory = principal \"$TEST_STORAGE_FACTORY_ID\";dfx_ledger = opt principal \"$DFX_LEDGER_CANISTER_ID\"; test_runner_nft = null; test_runner_nft_2 = null; test_runner_instant = null; test_runner_data = null; test_runner_utils = opt principal \"$TEST_RUNNER_UTIL_CANISTER_ID\"; test_runner_collection = null;test_runner_storage = null; test_runner_sale = null;})"

dfx canister install test_runner_utils --wasm ./.dfx/local/canisters/test_runner_utils/test_runner_utils.wasm.gz --mode=reinstall --argument "(principal  \"$DFX_LEDGER_CANISTER_ID\", principal  \"$DFX_LEDGER_CANISTER2_ID\")"
